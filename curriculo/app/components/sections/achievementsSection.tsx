...
export default function AchievementsSection(){
    ...
    return(
        <section id="achievements" className="bg-yellow">
            ...
            <h2 className="text-darkBlue text-center font-medium font-raleway text-[20px] lg:text-[24px]">As melhores são as que vêm naturalmente, como consequência da paixão pelo que fazemos</h2> 
                <div className="mt-5">
                    {Object.keys(achievements).map((year, achievs) => {
                        return(
                            <div key={year} className="font-raleway flex flex-col">
                                <h2 className="text-darkBlue bg-lightBlue text-center font-raleway text-[40px] lg:text-[50px] font-bold ">{year}</h2>
                                <div className="flex flex-row flex-wrap m-[2vw] justify-center">
                                    {achievements[year].map((achiev: Achievement) => {
                                        return(
                                            <div key={achiev.id} className={`${mainColors} m-4 sm:max-w-[40vw] lg:max-w-[25vw] group overflow-hidden flex flex-col justify-between transition-all duration-300 ease-out rounded-3xl border-[1px] shadow-2xl`}>
                                                ...
                                                <div className="transition-all duration-300 ease-out bg-darkBlue group-hover:bg-lightBlue text-yellow group-hover:text-darkBlue">
                                                    <span className="font-bold text-[32px] italic pl-[2vw]">{achiev.institution_name}</span>
                                                </div>
                                            </div>
                                        );
                                    })}
