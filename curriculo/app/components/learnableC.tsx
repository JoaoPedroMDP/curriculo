...
export default function LearnableC({learnable}: {learnable: any, onClickHandler?: any}){
    ...
    let textConfig = "text-nowrap text-center text-darkBlue group-hover:text-whiteBlue text-[18px]"

    return (
    <div className={`group flex flex-grow justify-center transition-color duration-200 hover:bg-darkBlue p-5`}>
        <p 
        className={`${underlineConfig} ${textConfig} after:bg-whiteBlue w-min transition-color duration-300 cursor-pointer`}
        >
            {learnable.name}
        </p>
    </div>
